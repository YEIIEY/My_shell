# 一、功能概述
My_Shell 是一个用 C++ 编写的简单命令行解释器，支持基本的命令执行、I/O 重定向和管道操作。它还支持执行简单的 shell 脚本。<br>
...
### 3.1. 用例图
#### 描述：
用例图展示了用户与My_Shell 系统之间的交互，主要包括用户输入命令、My_Shell 解析命令、执行命令并返回结果。<br>
#### 文字描述：
1. 用户：通过命令行界面与My_Shell 交互，输入命令并期望得到结果。<br>
2. My_Shell：接收用户输入的命令，解析并执行命令，并将结果返回给用户。<br>
   · 输入命令：用户输入命令字符串。<br>
   · 解析命令：My_Shell 解析命令字符串，将其分割为命令、参数和重定向信息。<br>
   · 执行命令：My_Shell 创建子进程并执行解析后的命令。<br>
   · 返回结果：将命令的执行结果返回给用户，输出到命令行界面。<br>
![用例图](https://github.com/YEIIEY/My_shell/assets/127741388/d7677ad1-2259-47fd-a675-a740bb78c0f5)

### 3.2.类图
#### 描述：
类图展示了 My_Shell 系统中的主要类及其属性和方法。<br>
#### 文字描述：
...
### 3.3. 时序图
#### 描述：
时序图展示了用户输入命令、My_Shell 解析并执行命令、然后返回结果的过程<br>
#### 文字描述：
1. 用户：输入命令到命令行界面。<br>
...
## 4. 错误处理
My_Shell 提供详细的错误信息，帮助用户快速定位和解决问题。主要错误包括：<br>
· 空命令错误。<br>
· I/O 文件格式错误。<br>
· 字符串引号错误。<br>
· 进程创建失败。<br>
